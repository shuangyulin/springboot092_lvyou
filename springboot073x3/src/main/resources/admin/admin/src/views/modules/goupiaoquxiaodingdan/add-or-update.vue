<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="购票时间" prop="goupiaoshijian">
          <el-input v-model="ruleForm.goupiaoshijian" 
              placeholder="购票时间" clearable  :readonly="ro.goupiaoshijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="购票时间" prop="goupiaoshijian">
              <el-input v-model="ruleForm.goupiaoshijian" 
                placeholder="购票时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    zongjine:{
      get: function () {
        return 1*this.ruleForm.piaoshu*this.ruleForm.jiage
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `goupiaoquxiaodingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `goupiaoquxiaodingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.goupiaoquxiaodingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
