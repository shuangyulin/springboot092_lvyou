<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="酒店价格" prop="jiudianjiage">
          <el-input v-model="ruleForm.jiudianjiage" 
              placeholder="酒店价格" clearable  :readonly="ro.jiudianjiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="酒店价格" prop="jiudianjiage">
              <el-input v-model="ruleForm.jiudianjiage" 
                placeholder="酒店价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    zongjine:{
      get: function () {
        return 1*this.ruleForm.fangjianshu*this.ruleForm.jiudianjiage*this.ruleForm.ruzhutianshu
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.suozaidiqu = true;
          ...
          if(o=='jiudianjiage'){
            this.ruleForm.jiudianjiage = obj[o];
	    this.ro.jiudianjiage = true;
            continue;
          }
          if(o=='ruzhushijian'){
        ...
        this.ruleForm.fangjianshu = 0
	this.ro.fangjianshu = false;
      }
      // 获取用户信息
      this.$http({
    ...
    onSubmit() {
      ...
      if(obj!=null) {
	      obj.fangjianshu = obj.fangjianshu - this.ruleForm.fangjianshu
	      if(obj.fangjianshu<0){
		this.$message.error("房间数不足");
		return
	      }
	      this.$http({
		  url: `${table}/update`,
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiudiandingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiudiandingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
