<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="市区" prop="shiqu">
          <el-input v-model="ruleForm.shiqu" 
              placeholder="市区" clearable  :readonly="ro.shiqu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="市区" prop="shiqu">
              <el-input v-model="ruleForm.shiqu" 
                placeholder="市区" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shiqu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shiquCrossAddOrUpdateFlag = false;
                  this.parent.search();
